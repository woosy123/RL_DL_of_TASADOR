...
class script:
    ...
    def get_cpu_usage(self):
        with open('get_cpu.txt','r') as f:
            cpu1 = float(f.readline().rstrip())
            cpu2 = float(f.readline().rstrip())
            cpu3 = float(f.readline().rstrip())
            cpu4 = float(f.readline().rstrip())
            cpu5 = float(f.readline().rstrip())
            cpu6 = float(f.readline().rstrip())
            cpu7 = float(f.readline().rstrip())
            cpu8 = float(f.readline().rstrip())
        cpu = cpu1+cpu2+cpu3+cpu4+cpu5+cpu6+cpu7+cpu8
        return round(cpu,2)
