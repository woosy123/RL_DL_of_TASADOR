...
try:
    ...
    df = pd.DataFrame(columns=["Episode", "Reward","time"])

# 새로운 보상을 엑셀에 추가하는 함수
def add_reward(episode, reward, time):
    df.loc[len(df)] = [episode, reward, time]
...
for i_episode in range(num_episodes):
    ...
    add_reward(i_episode + 1, float(episode_reward), end_time-start_time)
